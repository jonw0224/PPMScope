...
ih_1
	decfsz gbl_cntra, f					;Count down the trigger delay, 0x010101 implies minimum delay
		goto ih_1
	decfsz gbl_cntrb, f
		goto ih_1
	decfsz gbl_cntrc, f
...
Initial
	...
	movwf	gbl_cntrb		; selection having being set
	movwf	gbl_cntrc
	
Main
	MOVLF 0x01, gbl_cntra	;Set up trigger delay to minimum
	movwf gbl_cntrb
	movwf gbl_cntrc
	MOVLF 0x11, gbl_sampleRate2	;Set up sampleRate for 35 + 8*(256*16+16+1) =  32,929 cycles
	MOVLF 0x10, gbl_sampleRate1
	MOVLF D'12'*D'3', gbl_modetemp
	;bsf INTCON, INTE		;Interrupt on
	;bsf INTCON, GIE
	;goto $
	MGOTO SampleIntDrvn		;Start sampling
...
WaitComm_Rec							;***** Check for/Respond to Command "0xFA" (250) *****
	...
	call	PutByte
	call	ConfReceive
	movlw	0x01						;Set counters to 0x010101 for the case of
...
;Jonathan Weaver: Measurements from simulation
...
;(around 140 cycles to execute the timer1 interrupt.  Added some time for processing the serial port.)
;Maximum sample rate (at SAMPLERATE1:SAMPLERATE2 = 20) is 1,953 Hz.
;Waveform completion at that sample rate is 7.6 times per second
...
;Paul, I made some modifications here to fix the glitches and to make the sampling adhere to the XY method
;more precisely
...
;	nop
	...
	bsf		ADC_CONTROLPORT, RDD		;Stop conversion
;JWW - Another changed code section
...
;	incf	b0_sampleAddr, F			; ...2 times for 2 samples taken
	...
	movlw BUFFERMIN-1					;Check if filling last aquisition memory location in bank
...
;JWW - added condition for lower limit of Buffer address
irqst1_6
    MOVLF BUFFERMIN, b0_sampleAddr		
