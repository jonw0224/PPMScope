...
;*****BEGIN PM DSOscope CHANGE************************************************
;NEW VERSION CREATED:	A01
;	20120522.1 - by PM
...
CBLOCK	0x70			;Shared General Purpose Registers, accessible anywhere
	...
	cntr				;counter variable (use to throttle serial port for USB converter)
	spdata				;data to/from serial port
...
CONFRECEIVE macro
	...
	MOVFF spdata, triggerDelay1
	call GetByte
	MOVFF spdata, triggerDelay2
	call GetByte
	MOVFF spdata, triggerDelay3
	call GetByte					;Receive sample rate
	MOVFF spdata, sampleRate1
	call GetByte
	MOVFF spdata, sampleRate2
	endm

CONFSEND macro
	;Send number of datapoints (2 bytes)
	MOVLF high TOTALBUFFERSIZE, spdata
	call PutByte
	MOVLF low TOTALBUFFERSIZE, spdata
	call PutByte
	;Send clock frequency configuration
	MOVFF configLoc1, spdata
	call PutByte
	MOVFF triggerDelay1, spdata
	call PutByte
	MOVFF triggerDelay2, spdata
	call PutByte
	MOVFF triggerDelay3, spdata
	call PutByte
	MOVFF sampleRate1, spdata
	call PutByte
	MOVFF sampleRate2, spdata
	call PutByte
	endm

CHANNELCONFSEND macro
	MOVFF dataConfig, spdata		;Channel configuration
	call PutByte
	clrf spdata
	call PutByte					;Channel 1 offset
	call PutByte					;Channel 2 offset
	MOVFF triggerLevel, spdata
...
MainSelect
	...
	MGOTO WaitComm						;Invalid FreqMode = 12
	MGOTO WaitComm						;Invalid FreqMode = 13
	MGOTO WaitComm						;Invalid FreqMode = 14
	MGOTO WaitComm						;Invalid FreqMode = 15
...
Main
	...
	clrf modetempb						;Clear the modetempb for repetitive sampling
...
MainB
	...
	bcf STATUS, C
	rlf modetemp, f
	addwf modetemp, w					; and put the result into W register for later computed jump
...
MainWTrigB
	...
	skipEqLF B'11110011', spdata		;message is received
		goto MainWTrigB
	MOVLF B'11011100', spdata			;Respond
	call PutByte
...
WaitComm_Test						;Determine the command and the response
	skipEqLF B'11111000', spdata
		goto WaitComm_Conf
WaitComm_Data
	MOVLF B'11011100', spdata		;Send beginning of message
	call PutByte
	MOVLF BUFFERMIN, FSR			;Set pointer to beginning of queue
	MOVLF BUFFERSIZE, cntrb			;Set counter
WaitComm_SendDataA
	MOVFF INDF, spdata				;Get data ready to send
...
WaitComm_Conf
	skipEqLF B'11111001', spdata
		goto WaitComm_Rec
	MOVLF B'11011101', spdata		;Send beginning of message
	call PutByte	
	CONFSEND
	goto WaitComm

WaitComm_Rec
	skipEqLF B'11111010', spdata
		goto WaitComm				;No valid message sent
	MOVLF B'11011110', spdata		;Send beginning of message
	call PutByte
	CONFRECEIVE
...
GetByteSerialE					;No errors if we got here	
	MOVFF RCREG, spdata			;Read byte from serial FIFO
	retlw 0
