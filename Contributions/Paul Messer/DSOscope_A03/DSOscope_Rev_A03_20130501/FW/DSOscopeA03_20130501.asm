...
;		jitter that was occurring at the trigger position.
;
...
MainWaitTrig							;Trigger is enabled, set up interrupt
	...
	btfss	b0_triggerDelay0, 0x07		; to determine if we should be doing a pre-trigger acquisition		... comment out when testing
		goto	MainWTrigB				; if it is positive, go wait for the regular trigger				... comment out when testing
	movf	b0_triggerDelay3, W			;Assume that the PC app sends a pre-scaled value of 0xff for right	... comment out when testing
										; side, to 0x00 for left side, screen placement of trigger event
;	movlw	0xbf						;Load a fake value of a "negative trigger delay" (25% from right)	... uncomment when testing
...
;	nop
	...
	;The following check is to try and reduce the jitter seen on pre-trigger sampling.
    ; The theory being that a trigger event that happens while we are already servicing
...
;		goto irqst1_5
	...
	btfsc	b0_ptTriggerBank, SAMPLEBANK2
		xorlw 0x3f
		;Now account for the variouos banks
	btfsc	b0_ptTriggerBank, SAMPLEBANK1
		addlw (BUFFERSIZE * 1)
	btfsc	b0_ptTriggerBank, SAMPLEBANK2
		addlw (BUFFERSIZE * 3)
...
;	bcf		INTCON, INTF				;Last chance clear of the trigger interrupt flag,
;										; used for pre-trigger acquisitions to maximize samples
	...
	;	0xF9 (249) => Send a "0xDD" (221) followed by send of Configuration Information Field
	;	0xFA (250) => Send a "0xDE" (222) followed by receive of Configuration Information Field
	;	0xF3 (243) => Send a "0xDC" (220) acknowledgement of a Trigger Break missed and exits
	;
