...
;	20130211.1 / 20130302.1 - by PM																			<<<<Pre-trigger sampling change
...
CBLOCK	0x70
	...
	gbl_ptStatus			;pre-trigger acquistion status													<<<<Pre-trigger sampling change
ENDC
...
CBLOCK	0x60
	...
	b0_ptPtSampleCntr		;pre-trigger acquistion's post trigger sample counter							<<<<Pre-trigger sampling change
	b0_ptTriggerAddr		;sample's storage address when pre-trigger's trigger happens					<<<<Pre-trigger sampling change
	b0_ptTriggerBank		;sample's storage bank when pre-trigger's trigger happens						<<<<Pre-trigger sampling change
	b0_ptPlaceTriggerHere	;place trigger (0xfe = right side, 0x00 = left side, in 0x02 increments)		<<<<Pre-trigger sampling change
	b0_ptRotationCntr		;sample memory rotation counter													<<<<Pre-trigger sampling change
	b0_ptFsrTemp			;2nd level of fsr temp storage for pre-trigger									<<<<Pre-trigger sampling change
	b0_ptTemp0				;temporary storage																<<<<Pre-trigger sampling change
ENDC
...
IntHandler
	...
	; enable being active: ... THE PREVIOUS IS NOT TRUE ANYMORE WITH PRE-TRIGGER ACQUISITIONS!!! ...		<<<<Pre-trigger sampling change
	btfss	INTCON, INTE				;Check for trigger interrupt being enabled
		goto ih_2
	btfss	INTCON, INTF				;Check for a trigger interrupt										<<<<Pre-trigger sampling change
		goto ih_2						;																	<<<<Pre-trigger sampling change
	bcf		INTCON, INTE				;It was a trigger interrupt, turn off the trigger interrupt
										; for the duration of the acquisition (or the rest of the			<<<<Pre-trigger sampling change
										; acquisition in the case of Pre_Trigger acquisition)				<<<<Pre-trigger sampling change

	btfsc	gbl_ptStatus, RUNNINGBIT	;Check to see if we are NOT doing Pre-Trigger Sampling				<<<<Pre-trigger sampling change
...
MainSelect
	...
	MGOTO SampleStripChart				;FreqMode = 13
	MGOTO Main							;Invalid FreqMode = 14, goto beginning
	MGOTO Main							;Invalid FreqMode = 15

ih_pt									;This is for a "pre-trigger sampling" trigger interrupt				<<<<Pre-trigger sampling change
	banksel	BANK0						;																	<<<<Pre-trigger sampling change
	movf	b0_sampleBank, W			;Capture the current location that the interrupt driven				<<<<Pre-trigger sampling change
	movwf	b0_ptTriggerBank			; pre-trigger sample is being stored into							<<<<Pre-trigger sampling change
	movf	b0_sampleAddr, W			;																	<<<<Pre-trigger sampling change
	movwf	b0_ptTriggerAddr			;																	<<<<Pre-trigger sampling change
	bsf		gbl_ptStatus, TRIGGEREDBIT	;Flag that we have been triggered									<<<<Pre-trigger sampling change
	;fall through to also check for a timer1 interrupt. May as well try to get two birds with one stone!	<<<<Pre-trigger sampling change
...
;	bcf		STATUS, IRP		;Ensure indirect addressing of Bank0 and Bank1
	...
	movlw	0x01			;Initialize the "trigger delay counter" to minimium delay
	movwf	gbl_cntra		; for the initial case of "RUN" button pushed with the "TRIGGER OFF"
	movwf	gbl_cntrb		; selection having been set
	movwf	gbl_cntrc
	movwf	gbl_cntrd

Main
	bcf		STATUS, IRP					;Ensure indirect addressing of Bank0								<<<<Pre-trigger sampling change
	...
	clrf	gbl_ptStatus				;Clear the pre-trigger status register								<<<<Pre-trigger sampling change
...
MainWaitTrig							;Trigger is enabled, set up interrupt
	...
	bsf		INTCON, INTE				;Enable the RB0 specific interrupt
										...
										;Check for a negative value of the trigger delay					<<<<Pre-trigger sampling change
	btfss	b0_triggerDelay0, 0x07		; to determine if we should be doing a pre-trigger acquisition		<<<<Pre-trigger sampling change... comment out when testing
		goto	MainWTrigB				; if it is positive, go wait for the regular trigger				<<<<Pre-trigger sampling change... comment out when testing
	movf	b0_triggerDelay3, W			;Assume that the PC app sends a pre-scaled value of 0xff for right	<<<<Pre-trigger sampling change... comment out when testing
										; side, to 0x00 for left side, screen placement of trigger event	<<<<Pre-trigger sampling change
;	movlw	0xbf						;Load a fake value of a "negative trigger delay" (25% from right)	<<<<Pre-trigger sampling change... uncomment when testing
	andlw	0xfe						;Make it an even number												<<<<Pre-trigger sampling change
	movwf	b0_ptPlaceTriggerHere		;Put it into the "place trigger position here" variable				<<<<Pre-trigger sampling change
	xorlw	0xff						;Invert that, and make it even, to arrive at a count of				<<<<Pre-trigger sampling change
	andlw	0xfe						; samples to take after the trigger									<<<<Pre-trigger sampling change
	movwf	b0_ptPtSampleCntr
	bsf		gbl_ptStatus, RUNNINGBIT	;Set the pre-trigger acquisition state to running					<<<<Pre-trigger sampling change
	movf	gbl_modetemp, W				;Restore the freqmode for Mainselect's computed jump				<<<<Pre-trigger sampling change
	goto	MainSelect					; and, start the pre-trigger sampling								<<<<Pre-trigger sampling change
	

MainWTrigB
	bsf		INTCON, GIE					;Enable interrupts													<<<<Pre-trigger sampling change
MainWTrigB_1							;																	<<<<Pre-trigger sampling change
	call	GetByte						;Wait for the trigger unless a "break trigger"
	skipEqLF B'11110011', gbl_spdata	;message is received
		goto	MainWTrigB_1			;																	<<<<Pre-trigger sampling change
...
WaitComm_Data							;***** Check for/Respond to Command "0xF8" (248) *****
	...
	bcf		STATUS, IRP					;Set indirection to bank 0, FSR7 cleared in previous instruction	<<<<Pre-trigger sampling change
WaitComm_SendDataA
...
WaitComm_Rec							;***** Check for/Respond to Command "0xFA" (250) *****
	...
	call	PutByte
	call	ConfReceive
	movlw	0x01						;Set counters to 0x01010101 for the case of
	movwf	gbl_cntra					;  "TRIGGER OFF" having been set
	movwf	gbl_cntrb
	movwf	gbl_cntrc
	movwf	gbl_cntrd
	return
...
;This code section consists of three routines:																<<<<Pre-trigger sampling change
...
;		sampling interrupts
; (3) A routine to rotate the acquired sample memory to align it for proper									<<<<Pre-trigger sampling change
;		pre-trigger display																					<<<<Pre-trigger sampling change
...
irqst1_2
	...
	btfss	b0_sampleBank, 4			;Check to see if done with this acquisitions sampling
		goto irqst1_3					;Not done, so keep going...											<<<<Pre-trigger sampling change
	btfss	gbl_ptStatus, RUNNINGBIT	;Check to see if we are doing a pre-trigger sampling				<<<<Pre-trigger sampling change
		goto irqst1_4					;Done with a regular trigger's sampling								<<<<Pre-trigger sampling change
	clrf	b0_sampleBank				;Pre-trigger sampling, so just set the bank back to zero			<<<<Pre-trigger sampling change
	bsf		b0_sampleBank, SAMPLEBANK0	;																	<<<<Pre-trigger sampling change

irqst1_3
		;If we are doing pre-trigger sampling AND if we have already seen a trigger, then					<<<<Pre-trigger sampling change
		; check to see if the required number of samples after a trigger event has been completed			<<<<Pre-trigger sampling change
		; If it has, then stop the acquisition so we don't overwrite the pre-trigger samples				<<<<Pre-trigger sampling change
	btfss	gbl_ptStatus, RUNNINGBIT	;																	<<<<Pre-trigger sampling change
		goto irqst1_5					;																	<<<<Pre-trigger sampling change
	btfss	gbl_ptStatus, TRIGGEREDBIT	;																	<<<<Pre-trigger sampling change
		goto irqst1_5					;																	<<<<Pre-trigger sampling change
	decf	b0_ptPtSampleCntr, F		;																	<<<<Pre-trigger sampling change
	decfsz	b0_ptPtSampleCntr, F		;																	<<<<Pre-trigger sampling change
		goto irqst1_5					;																	<<<<Pre-trigger sampling change

		;Now calculate an amount to rotate the sample memory to align the trigger with						<<<<Pre-trigger sampling change
		; its requested position on the screen																<<<<Pre-trigger sampling change
		;Convert the captured address/bank of where the trigger event was stored							<<<<Pre-trigger sampling change
		; into a meaningful linear address value... Wow is this confusing!  Lets hope we					<<<<Pre-trigger sampling change
		; get it right!																						<<<<Pre-trigger sampling change
		;First account for whether the address was in an incrementing or a decrementing bank				<<<<Pre-trigger sampling change
	movlw	BUFFERMIN					;																	<<<<Pre-trigger sampling change
	subwf	b0_ptTriggerAddr, W			;																	<<<<Pre-trigger sampling change
	btfsc	b0_ptTriggerBank, SAMPLEBANK1	;																<<<<Pre-trigger sampling change
		xorlw 0x3f						;																	<<<<Pre-trigger sampling change
	btfsc	b0_ptTriggerBank, SAMPLEBANK2	;																<<<<Pre-trigger sampling change
		xorlw 0x3f						;																	<<<<Pre-trigger sampling change
		;Now account for the variouos banks																	<<<<Pre-trigger sampling change
	btfsc	b0_ptTriggerBank, SAMPLEBANK1	;																<<<<Pre-trigger sampling change
		addlw (BUFFERSIZE * 1)			;																	<<<<Pre-trigger sampling change
	btfsc	b0_ptTriggerBank, SAMPLEBANK2	;																<<<<Pre-trigger sampling change
		addlw (BUFFERSIZE * 3)			;																	<<<<Pre-trigger sampling change
	btfsc	b0_ptTriggerBank, SAMPLEBANK3	;																<<<<Pre-trigger sampling change
		addlw (BUFFERSIZE * 2)			;																	<<<<Pre-trigger sampling change
	addlw	0xfe						;Subtract 2 from the value to account for the post increment		<<<<Pre-trigger sampling change
		;Then calculate out how much rotation is needed to align the trigger event with						<<<<Pre-trigger sampling change
		; the requested screen position																		<<<<Pre-trigger sampling change
	subwf	b0_ptPlaceTriggerHere, W	;																	<<<<Pre-trigger sampling change
	andlw	0xfe						;Make sure it is an even number										<<<<Pre-trigger sampling change
	movwf	b0_ptRotationCntr			;																	<<<<Pre-trigger sampling change
		;Now decide whether rotating right or rotating left gets us aligned the quickest					<<<<Pre-trigger sampling change
	btfss	b0_ptRotationCntr, 7		;																	<<<<Pre-trigger sampling change
		goto irqst1_3_1					;Fastest is to rotate right											<<<<Pre-trigger sampling change
	movlw	0xfe						;Fastest is to rotate left, make some counter adjustments			<<<<Pre-trigger sampling change
	xorwf	b0_ptRotationCntr, F		;																	<<<<Pre-trigger sampling change
	incf	b0_ptRotationCntr, F		;																	<<<<Pre-trigger sampling change
	incf	b0_ptRotationCntr, F		; 																	<<<<Pre-trigger sampling change
	call	RotateSampleMemLeft			;																	<<<<Pre-trigger sampling change
	goto	irqst1_3_2					;																	<<<<Pre-trigger sampling change
irqst1_3_1								;																	<<<<Pre-trigger sampling change
	call	RotateSampleMemRight		;																	<<<<Pre-trigger sampling change
irqst1_3_2								;																	<<<<Pre-trigger sampling change
	bsf		gbl_ptStatus, DATAREADYBIT	;Set the pre-trigger acquisition's data is now ready flag			<<<<Pre-trigger sampling change

irqst1_4								;																	<<<<Pre-trigger sampling change
...
irqst1_5								;																	<<<<Pre-trigger sampling change
	banksel	BANK0
...
SampleIntDrvn
	...
	bcf		INTCON, INTF				;Last chance clear of the trigger interrupt flag,					<<<<Pre-trigger sampling change
										; used for pre-trigger acquisitions to maximize samples				<<<<Pre-trigger sampling change
...
sid_GetByteE							;No errors if we got here	
	...
	btfss	gbl_ptStatus, RUNNINGBIT	;Check to see if we are/were running a pre-trigger acquisition		<<<<Pre-trigger sampling change
		goto sid_WaitComm_Data			;																	<<<<Pre-trigger sampling change
	btfss	gbl_ptStatus, DATAREADYBIT	;Check to see if pre-trigger acquisition data is ready				<<<<Pre-trigger sampling change
		goto sid_GetByte				; if not, ignore commands at this time!								<<<<Pre-trigger sampling change
...
sid_Exit
	bcf		INTCON, GIE					;Disable interrupts													<<<<Pre-trigger sampling change
	banksel	BANK0
	bsf		ADC_PORT, CS				;Unselect MAX118
;	bcf		STATUS, IRP					;Set indirection to bank 0 and/or bank 1
...
;-  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -								<<<<Pre-trigger sampling change
RotateSampleMemLeft						;																	<<<<Pre-trigger sampling change
	;Rotate the data stored in the sample buffer memory by 0xNN positions to the left						<<<<Pre-trigger sampling change
	;Used after a pre-trigger sampling to align the data in the buffer memory								<<<<Pre-trigger sampling change
	banksel	BANK0						;																	<<<<Pre-trigger sampling change
	movf	FSR, W						;Save the current FSR												<<<<Pre-trigger sampling change
	movwf	b0_ptFsrTemp				;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
rsmL_1									;																	<<<<Pre-trigger sampling change
	movf	b0_ptRotationCntr, F		;Check to see if no memory rotation is needed						<<<<Pre-trigger sampling change
	btfsc	STATUS, Z					;																	<<<<Pre-trigger sampling change
		goto rsm_exit					;																	<<<<Pre-trigger sampling change
	movlw	BUFFERMIN					;Not done, so setup the indirection register						<<<<Pre-trigger sampling change
	movwf	FSR							;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
	;Start the rotation with BANK0																			<<<<Pre-trigger sampling change
	bcf		STATUS, IRP					;																	<<<<Pre-trigger sampling change
	bcf		FSR, 7						;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;																	<<<<Pre-trigger sampling change
	movwf	b0_ptTemp0					;Save very first location, to later be put into very last location	<<<<Pre-trigger sampling change
	call	rsm_slide_lower				;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
	;Set BANK1							;																	<<<<Pre-trigger sampling change
	bsf		FSR, 7						;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;Get the first location in bank 1 (is the top address of bank)		<<<<Pre-trigger sampling change
	bcf		FSR, 7						;Set indirection back to bank 0										<<<<Pre-trigger sampling change
	movwf	INDF						;Deposit bank 1's first location into bank 0's last location		<<<<Pre-trigger sampling change
	bsf		FSR, 7						;Now go back to bank 1												<<<<Pre-trigger sampling change
	call	rsm_slide_higher			;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
	;Set BANK3							;																	<<<<Pre-trigger sampling change
	bsf		STATUS, IRP					;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;Get the first location in bank 3 (is the bottom address bank)		<<<<Pre-trigger sampling change
	bcf		STATUS, IRP					;Set indirection back to bank 1										<<<<Pre-trigger sampling change
	movwf	INDF						;Deposit bank 3's first location into bank 1's last location		<<<<Pre-trigger sampling change
	bsf		STATUS, IRP					;Now go back to bank 3												<<<<Pre-trigger sampling change
	call	rsm_slide_lower				;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
    ;Set BANK2							;																	<<<<Pre-trigger sampling change
	bcf		FSR, 7						;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;Get the first location in bank 2 (is the top address of bank)		<<<<Pre-trigger sampling change
	bsf		FSR, 7						;Set indirection back to bank 3										<<<<Pre-trigger sampling change
	movwf	INDF						;Deposit bank 2's first location into bank 3's last location		<<<<Pre-trigger sampling change
	bcf		FSR, 7						;Now go back to bank 2												<<<<Pre-trigger sampling change
	call	rsm_slide_higher			;																	<<<<Pre-trigger sampling change

	movf	b0_ptTemp0, W				;Now put the very first location into the very last location		<<<<Pre-trigger sampling change
	movwf	INDF						;																	<<<<Pre-trigger sampling change
	decfsz	b0_ptRotationCntr, F		;Check to see if memory has been rotated enough						<<<<Pre-trigger sampling change
		goto rsmL_1						;																	<<<<Pre-trigger sampling change
	goto	rsm_exit


RotateSampleMemRight					;																	<<<<Pre-trigger sampling change
	;Rotate the data stored in the sample buffer memory by 0xNN positions to the right						<<<<Pre-trigger sampling change
	;Used after a pre-trigger sampling to align the data in the buffer memory								<<<<Pre-trigger sampling change
	banksel	BANK0						;																	<<<<Pre-trigger sampling change
	movf	FSR, W						;Save the current FSR												<<<<Pre-trigger sampling change
	movwf	b0_ptFsrTemp				;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
rsmR_1									;																	<<<<Pre-trigger sampling change
	movf	b0_ptRotationCntr, F		;Check to see if no memory rotation is needed						<<<<Pre-trigger sampling change
	btfsc	STATUS, Z					;																	<<<<Pre-trigger sampling change
		goto rsm_exit					;																	<<<<Pre-trigger sampling change
	movlw	BUFFERMIN					;Not done, so setup the indirection register						<<<<Pre-trigger sampling change
	movwf	FSR							;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
	;Start the rotation with BANK2																			<<<<Pre-trigger sampling change
	bsf		STATUS, IRP					;																	<<<<Pre-trigger sampling change
	bcf		FSR, 7						;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;																	<<<<Pre-trigger sampling change
	movwf	b0_ptTemp0					;Save very first location, to later be put into very last location	<<<<Pre-trigger sampling change
	...
	;Set BANK3							;																	<<<<Pre-trigger sampling change
	bsf		FSR, 7						;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;Get the last location in bank 3 (is the top address of bank)		<<<<Pre-trigger sampling change
	bcf		FSR, 7						;Set indirection back to bank 2										<<<<Pre-trigger sampling change
	movwf	INDF						;Deposit bank 3's last location into bank 2's first location		<<<<Pre-trigger sampling change
	bsf		FSR, 7						;Now go back to bank 3												<<<<Pre-trigger sampling change
	call	rsm_slide_higher			;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
	;Set BANK1							;																	<<<<Pre-trigger sampling change
	bcf		STATUS, IRP					;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;Get the last location in bank 1 (is the bottom address of bank)	<<<<Pre-trigger sampling change
	bsf		STATUS, IRP					;Set indirection back to bank 3										<<<<Pre-trigger sampling change
	movwf	INDF						;Deposit bank 1's last location into bank 3's first location		<<<<Pre-trigger sampling change
	bcf		STATUS, IRP					;Now go back to bank 1												<<<<Pre-trigger sampling change
	call	rsm_slide_lower				;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
    ;Set BANK0							;																	<<<<Pre-trigger sampling change
	bcf		FSR, 7						;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;Get the last location in bank 0 (is the top address of bank)		<<<<Pre-trigger sampling change
	bsf		FSR, 7						;Set indirection back to bank 1										<<<<Pre-trigger sampling change
	movwf	INDF						;Deposit bank 0's last location into bank 1's first location		<<<<Pre-trigger sampling change
	bcf		FSR, 7						;Now go back to bank 0												<<<<Pre-trigger sampling change
	call	rsm_slide_higher			;																	<<<<Pre-trigger sampling change

	movf	b0_ptTemp0, W				;Now put the very first location into the very last location		<<<<Pre-trigger sampling change
	movwf	INDF						;																	<<<<Pre-trigger sampling change
	decfsz	b0_ptRotationCntr, F		;Check to see if memory has been rotated enough						<<<<Pre-trigger sampling change
		goto rsmR_1						;																	<<<<Pre-trigger sampling change
										;																	<<<<Pre-trigger sampling change
rsm_exit								;																	<<<<Pre-trigger sampling change
	movf	b0_ptFsrTemp, W				;Restore the previous FSR											<<<<Pre-trigger sampling change
	movwf	FSR							;																	<<<<Pre-trigger sampling change
	return								;																	<<<<Pre-trigger sampling change

rsm_slide_lower							;																	<<<<Pre-trigger sampling change
	incf	FSR, F						;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;																	<<<<Pre-trigger sampling change
	decf	FSR, F						;																	<<<<Pre-trigger sampling change
	movwf	INDF						;																	<<<<Pre-trigger sampling change
	incf	FSR, F						;																	<<<<Pre-trigger sampling change
	movlw	BUFFERMAX					;																	<<<<Pre-trigger sampling change
	xorwf	FSR, W						;																	<<<<Pre-trigger sampling change
	andlw	0x7f						;Mask out the FSR7 bit from the comparison							<<<<Pre-trigger sampling change
	btfss	STATUS, Z					;																	<<<<Pre-trigger sampling change
		goto rsm_slide_lower			;																	<<<<Pre-trigger sampling change
	return								;																	<<<<Pre-trigger sampling change

rsm_slide_higher						;																	<<<<Pre-trigger sampling change
	decf	FSR, F						;																	<<<<Pre-trigger sampling change
	movf	INDF, W						;																	<<<<Pre-trigger sampling change
	incf	FSR, F						;																	<<<<Pre-trigger sampling change
	movwf	INDF						;																	<<<<Pre-trigger sampling change
	...
	andlw	0x7f						;Mask out the FSR7 bit from the comparison							<<<<Pre-trigger sampling change
	btfss	STATUS, Z					;																	<<<<Pre-trigger sampling change
		goto rsm_slide_higher			;																	<<<<Pre-trigger sampling change
	return								;																	<<<<Pre-trigger sampling change




;SampleStripChart is used to send a continuous stream of ADC samples to the PC app.
...
; the interrupt rate, determined by sampleRate0:sampleRate2, below the effective
...
;-----------------------------------------------------------------------------
SampleStripChart
	goto	Main						;Still being implemented
